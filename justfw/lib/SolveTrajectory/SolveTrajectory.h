...
#ifndef JUSTFW_SOLVETRAJECTORY_H
#define JUSTFW_SOLVETRAJECTORY_H
#ifndef PI
...
//单方向空气阻力模型
extern float monoDirectionalAirResistanceModel(struct SolveTrajectoryParams *st,float s, float v, float angle);
//完全空气阻力模型
extern float completeAirResistanceModel(struct SolveTrajectoryParams *st,float s, float v, float angle);
//pitch弹道补偿
extern float pitchTrajectoryCompensation(struct SolveTrajectoryParams *st,float s, float y, float v);
//根据最优决策得出被击打装甲板 自动解算弹道
extern void autoSolveTrajectory(struct SolveTrajectoryParams *st,float *pitch, float *yaw, float *aim_x, float *aim_y, float *aim_z);


#endif // JUSTFW_SOLVETRAJECTORY_H
